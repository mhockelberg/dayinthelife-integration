...
```bash
http://location-service-international.apps.littleton-eedb.openshiftworkshop.com
```
...
1. In you command line terminal or in your Che terminal enter the following CURL command:
	...
	*Replace USERX as your user id, such as user1, user26, and OPENSHIFT_APP_URL, if you are not sure, check with your instructor*
	...
	```bash
	curl -X POST http://threescale-automate-international.apps.littleton-eedb.openshiftworkshop.com/threescale/automate/{YOUR_API_TOKEN}/{USERX}/{OPENSHIFT_APP_URL}
	```

	For example:

	```bash
	curl -X POST http://threescale-automate-international.apps.littleton-eedb.openshiftworkshop.com/threescale/automate/829405ec3d2dd0f91aa8435347827135c323c69757dd2dfb49ed41aa8ceb13ef/userX/apps.littleton-eedb.openshiftworkshop.com
	```

1. The following should be returned as the result:

	```bash
	API automated, DONE! REMEBER THIS ADDRESS FOR NEXT LAB: PLAN_URL :[ https://userx.apps.littleton-eedb.openshiftworkshop.com/signup?plan_ids[]=XXX ]
	```
...
1. Now that we are on the *Integration* page, we need to update the *Authentication Settings* to the correct value.  Update the *OpenID Connect Issuer* entry to ensure that it starts with **https**.  Also verify it has your correct userID in the URL:
	...
	```bash
	https://3scale-admin:82c0dfa9-4adb-41a7-9ac9-a03159d585c0@sso-sso.apps.littleton-eedb.openshiftworkshop.com/auth/realms/user50
	```
...
```org.apache.camel.http.common.HttpOperationFailedException: HTTP operation failed invoking https://{USERX}-admin.apps.littleton-eedb.openshiftworkshop.com/admin/api/services.xml with statusCode: 422```

This may be due to a failed build. Troubleshoot by deleting SSO Location API.
SSO Location API can be located here:

* In your browser, `https://{USERX}-admin.apps.littleton-eedb.openshiftworkshop.com`
